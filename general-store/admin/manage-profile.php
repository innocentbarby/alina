...
$resSel = mysqli_query ($conn ,"SELECT * FROM `admins` WHERE `id`='$id'");
...
if(isset($_GET["id"]) && $_GET["id"]>0){
  $res = mysqli_query($conn, "UPDATE `admins` SET `name` = '$name',email` = '$email',password` = '$password',`image`='$image'   WHERE `id` = '$id'"); 
}else{
  $resCheckTitle = mysqli_query($conn,"SELECT * FROM `admins` WHERE `name`='$name'");
...
// die();
    ...
    $res = mysqli_query($conn, "INSERT INTO`admins`(`name`,`email`,`password`,`image`)VALUES('$name','$email','$password','$image')");
  }
}
if($res){
...
?>
     ...
     <a href="profile.php" class="btn btn-sm btn-primary"><i class="fas fa-plus me-1"></i>view<?php $pageName; ?></a>
      </div>
      <div class="card-body">
